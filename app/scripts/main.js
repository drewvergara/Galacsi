...
//		colors = [];		//array with all gradient steps
	...
	var 	amount = 32, 		//how many steps
		start = [255, 255, 255],	//startcolor, RGB
		end = [255, 255, 255],	//endcolor, RGB
		steps = [],		//a single gradient step
		colors = [];		//array with all gradient steps
...
(function($){
    ...
    console.log(Math.round(size.height * 0.85));
    
    var container = document.getElementById('container');
    var renderer = new FSS.SVGRenderer();
    var scene = new FSS.Scene();
    var light = new FSS.Light('#111122', '#F6BBFD');
    var geometry = new FSS.Plane(Math.round(size.height * 0.85), Math.round(size.height * 0.85), 15, 15);
    var material = new FSS.Material('#FFFFFF', '#FFFFFF');
    var mesh = new FSS.Mesh(geometry, material);
    var now, start = Date.now();
    
    function initialise() {
    var renderContainer = $('#container');
    renderContainer.css('-webkit-border-radius', Math.round(size.height * 0.85) + 'px');
    renderContainer.css('top', Math.round(size.height * 0.70) + 'px');
    renderContainer.css('width', Math.round(size.height * 0.85) + 'px');
    ...
    function resize() {
      //renderer.setSize(container.offsetWidth, container.offsetHeight);
        renderer.setSize(Math.round(size.height * 0.85), Math.round(size.height * 0.85));
    }
    
    function animate() {
      now = Date.now() - start;
      light.setPosition(300*Math.sin(now*0.001), 300*Math.cos(now*0.0005), 60);
    ...
    var svgContainer = $('svg').css('-webkit-border-radius', Math.round(size.height * 0.85) + 'px');    
})(jQuery);